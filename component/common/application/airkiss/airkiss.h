...
#define AIRKISS_INFO		2
#define AIRKISS_DEBUG		3
#define AIRKISS_NONE		0xFF
#define AIRKISS_DEBUG_LEVEL AIRKISS_INFO

#if (AIRKISS_DEBUG_LEVEL== AIRKISS_NONE)
#define airkiss_printf(level, fmt, arg...)
#else
#define airkiss_printf(level, fmt, arg...)     \
do {\
	if (level <= AIRKISS_DEBUG_LEVEL) {\
		if (level <= AIRKISS_ERROR) {\
			printf("\r\nERROR: " fmt, ##arg);\
		} \
		else {\
			printf("\r\n"fmt, ##arg);\
