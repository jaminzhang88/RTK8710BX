...
#if 0
void cfg_net_cli(int argc, char **argv)
{
  char *argv2[] = { "wifi", "profile","0" };
  char *argv3[] = { "wifi", "profile","erase" };
    
  if (argc == 2 && !strcmp(argv[1], "airkiss"))
  {
    adw_wifi_cli(3, argv2);
    adw_wifi_cli(3, argv3);
    vTaskDelay(800);//延时等待进入ap模式
...
{ 
  if (argc == 2 && !strcmp(argv[1], "airkiss"))
  {
    ...
    vTaskDelay(800);//等待进入ap模式
    adw_wifi_cli_scan(NULL);
    vTaskDelay(800);//等待扫描完成
    adw_wifi_start_aks_ext(NULL);
    return;
  }
  ...
  if (argc == 2 && !strcmp(argv[1], "aks")) 
  {
    //wifi/setup_mode/enable
    char *argv[] = { "wifi", "setup_mode", "airkiss", "enable" };
  ...
  if (argc == 2 && !strcmp(argv[1], "aks3")) //切换到airkiss
  {
    //wifi/setup_mode/enable
    ...
    if (argc == 2 && !strcmp(argv[1], "ap"))   //切换到ap
  {
    //wifi/setup_mode/enable
...
static const char cfg_net_cli_help[] = "<airkiss|ap>";
...
static const char ada_conf_dsn_cli_help[] = "<dsn sa|us|eu>";
...
static const struct cmd_info client_conf_cmds[] = {
    CMD_INIT("cfg_net", cfg_net_cli, cfg_net_cli_help),
	...
	//CMD_INIT("dsn", ada_conf_dsn_cli, ada_conf_dsn_cli_help),
#ifdef AYLA_WIFI_SUPPORT
