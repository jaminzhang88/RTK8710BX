...
static void sched_actions_reset(struct sched_state *st)
{
	...
	if (st->action_table_ct < st->action_en_ct) {
		new = (struct sched_action *)malloc(sizeof(*new) * st->action_en_ct);
		if (!new) {
...
run_schedules:
	...
	do {
		...
		SCHED_LOGF(LOG_DEBUG2, "next event %u", next_event);
...
move_on:
	...
	SCHED_LOGF(LOG_DEBUG2, "earliest event %u", earliest_event);
	...
	SCHED_LOGF(LOG_DEBUG, "scheduling timeout for %d",
