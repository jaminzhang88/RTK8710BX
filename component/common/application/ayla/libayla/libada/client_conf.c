...
#include "flash_api.h"
#define OEM_DSN_SA     "XXXXXXX"
#define OEM_DSN_US     "XXXXXXX"
#define OEM_DSN_EU     "XXXXXXX"


#define OEM_ID_SA     "XXXXXXX"
#define OEM_ID_US     "XXXXXXX"
#define OEM_ID_EU     "XXXXXXX"

#define OEM_MODEL_SA     "XXXXXXX"
#define OEM_MODEL_US     "XXXXXXX"
#define OEM_MODEL_EU     "XXXXXXX"

#define OEM_KEY_SA     "XXXXXXX"
#define OEM_KEY_US     "XXXXXXX"
#define OEM_KEY_EU     "XXXXXXX"
...
extern unsigned int  ADDRESS_PUBKEY;
...
int adap_conf_pub_key_get1(void *buf, size_t len)
{   
	flash_t  flash;
    char pub_key[500]=NULL;
    memset(pub_key,0,500);
    flash_stream_read(&flash,ADDRESS_OEMKEY,344,pub_key);
    printf("\noemkeybefore:%s\n\n",pub_key);
    size_t outlen = len;
    
	if (net_base64_decode(pub_key, 344, buf, &outlen)) {
		client_log(LOG_ERR "oem_key decode failed");
		return -1;
	}
	return (int)outlen;
}
...
void ada_conf_file_cli(int argc, char **argv)
{
	...
	if(len>325){
                ...
                if(*((char *)file->buf)=='s'){
                   ...
                   flash_t  flash;
                   u8 return_value=flash_stream_write(&flash,  ADDRESS_PUBKEY, len, ((char *)file->buf));
                   memset((char *)file->buf,0,len);
                   vTaskDelay(200);
                   printf("\npara set ok\r\n");
                   return ;
...
#if 0
void ada_conf_dsn_cli(int argc, char **argv)
{
	...
	if (!strcmp(argv[1], "sa")) {
		...
		//进入setupmode
	        ...
	        //写入dsn
	        char *argv1[] = { "id", "dev_id",OEM_DSN_SA };
                ada_conf_id_cli(3,argv1);
                //写入public key
                
	        //写入server region
                char *argv3[] = { "client","server","region" ,"cn"};
                demo_client_cmd(4,argv3);
	        //写入oem id sa 
                char *argv4[] = { "oem",OEM_ID_SA };
                ada_conf_oem_cli(2,argv4);
	        //写入oem model sa
                char *argv5[] = { "oem","model",OEM_MODEL_SA};
                ada_conf_oem_cli(3,argv5);
	        //写入oem key  sa
                char *argv6[] = { "oem", "key",OEM_KEY_SA};
                ada_conf_oem_cli(3,argv6);
	        //保存配置
                char *argv7[] = { "conf", "save" };
                conf_cli(2, argv7);//保存
	        //退出setup_mode
	        char *argv8[] = { "setup_mode", "disable" };
	        demo_setup_mode_cmd(3,argv0);
		...
		//进入setupmode
	        ...
	        demo_setup_mode_cmd(3,argv0);
	        //写入dsn
	        char *argv1[] = { "id", "dev_id",OEM_DSN_US};
                ada_conf_id_cli(3,argv1);
                //写入public key
                
	        //写入server region
                char *argv3[] = { "client","server","region" ,"us"};
                demo_client_cmd(4,argv3);
	        //写入oem id sa 
                char *argv4[] = { "oem",OEM_ID_US };
                ada_conf_oem_cli(2,argv4);
	        //写入oem model sa
                char *argv5[] = { "oem","model",OEM_MODEL_US};
                ada_conf_oem_cli(3,argv5);
	        //写入oem key  sa
                char *argv6[] = { "oem", "key",OEM_KEY_US};
                ada_conf_oem_cli(3,argv6);
	        //保存配置
                char *argv7[] = { "conf", "save" };
                conf_cli(2, argv7);//保存
		...
		//进入setupmode
	        ...
	        demo_setup_mode_cmd(3,argv0);
	        //写入dsn
	        char *argv1[] = { "id", "dev_id",OEM_DSN_EU };
                ada_conf_id_cli(3,argv1);
                //写入public key
	        ...
	        //写入server region
                ...
                demo_client_cmd(4,argv3);
	        //写入oem id sa 
                char *argv4[] = { "oem",OEM_ID_EU };
                ada_conf_oem_cli(2,argv4);
	        //写入oem model sa
                char *argv5[] = { "oem","model",OEM_MODEL_EU};
                ada_conf_oem_cli(3,argv5);
	        //写入oem key  sa
                char *argv6[] = { "oem", "key",OEM_KEY_EU};
                ada_conf_oem_cli(3,argv6);
	        //保存配置
                char *argv7[] = { "conf", "save" };
                conf_cli(2, argv7);//保存
		...
		printcli("setting %s not supported", argv[0]);
	}
}
#endif
