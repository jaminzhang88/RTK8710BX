...
static void http_client_retry(struct http_client *hc)
{
	if (++hc->retries <= hc->retry_limit) {
		HTTP_CLIENT_DEBUG(hc, LOG_DEBUG2, "retry %d", hc->retries);
	...
	default:
		...
		if (payload || len) {
			HTTP_CLIENT_LOGF(hc, LOG_WARN,
			    "unexp recv %d bytes state %d", len, hc->state);
		}
		return len;
	}
	if (pcb != hc->pcb) {
...
http_parse:
		...
		HTTP_CLIENT_DEBUG(hc, LOG_DEBUG, "HTTP status = %d", status);
		...
		default:
			...
			if (hc->host[0] == '\0') {
				...
				HTTP_CLIENT_LOGF(hc, LOG_DEBUG,
				    "HTTP status %d recved from %d.%d.%d.%d",
				    status, bp[0], bp[1], bp[2], bp[3]);
			} else {
				HTTP_CLIENT_LOGF(hc, LOG_DEBUG,
				    "HTTP status %d recved from %s",
				    status, hc->host);
			}
			goto error_close;
			break;
