					...
					void *buf, size_t len)
{
	...
	if (buf) {
		...
		client_log(LOG_DEBUG "ota recv %d+%d/%d",
...
int client_ota_fetch_image(struct client_state *state)
{
	...
	if (state->ota_server.lan) {
		...
		client_log(LOG_DEBUG "OTA get from %s %d-%d", hc->host,
		    off, off + cnt);
		snprintf(range, sizeof(range), "bytes=%d-%d", off, off + cnt);
		hdr = &range_hdr;
	}
	ASSERT(hc);
