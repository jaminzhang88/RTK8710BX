...
struct adw_wifi_event_handler adw_wifi_event_handler;
				...
				void *arg)
{
	struct adw_wifi_event_handler *hp = &adw_wifi_event_handler;
        if(hp->handler)
        {
            printf("hp->handler!=NULL\n\r");
        }
        else
        {
            printf("hp->handler==NULL\n\r");
        }
	ASSERT(!hp->handler);//已注册则报错
	hp->handler = handler;
	hp->arg = arg;
}
 ...
 */
extern int  flag_join_success,flag_connect_fail;
...
{
	//adw_wifi_stop_aks_ext(NULL);   //airkiss超时自动进入ap模式
}
...
static int adw_wifi_alt_setup(struct adw_ssid *ssid, u8 *key, u16 key_len)
{
	...
	adw_lock();
	scan = adw_wifi_scan_lookup_ssid(wifi, ssid, 0);//WICED_SECURITY_UNKNOWN
	if (!scan) {
...
int adw_wifi_aks_setup(u8 *ssid,u8 ssid_len, u8 *key, u16 key_len)
{
  ...
  set.len = ssid_len;
  memcpy(set.id,ssid,set.len);
  
  adw_wifi_alt_setup(&set,key,key_len);
}
...
void adw_wifi_init(void)
{
	...
	timer_init(&adw_wifi_scan_timer, adw_wifi_rescan);//重新扫描wifi
	...
	net_callback_init(&adw_wifi_cbmsg_delayed, adw_wifi_step_arm_timer, wifi);
