...
static enum ada_err stream_tcp_connected(struct stream_pcb *sp)
{
	...
	if (sp->enable) {
		...
		mbedtls_ssl_set_bio(&sp->ssl, &sp->ssl_fd, mbedtls_net_send,
		    mbedtls_net_recv, mbedtls_net_recv_timeout); /* RTLTODO: timeout */
		   mbedtls_ssl_conf_read_timeout(&sp->ssl_cfg, 20000);
             //end
