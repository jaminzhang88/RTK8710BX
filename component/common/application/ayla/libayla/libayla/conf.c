...
unsigned int  ADDRESS=0x1d0000;
unsigned int  ADDRESS_PUBKEY=0x1e0000;
unsigned int  ADDRESS_OEMKEY=0x1f0000;
...
static void conf_tlv_fmt(char *obuf, size_t len, struct ayla_tlv *tlv)
{
	...
	case ATLV_FORMAT:
		if (type == ATLV_UINT) {
			...
			snprintf(obuf, len, "%d= 0x%x", val, val);
		} else if (type == ATLV_BOOL) {
			val = conf_get_uint_common(tlv);
			snprintf(obuf, len, "%x", val);
		} else {
			sval = conf_get_int_common(tlv);
			snprintf(obuf, len, "%d = 0x%x", sval, sval);
		}
		break;
			...
			struct ayla_tlv *match)
{
	...
	for (off = 0; off < len; ) {
		...
		if (CONF_SHOW_ALL) {
			snprintf(off_buf, sizeof(off_buf), "%x %s", tlv_off,
			    type);
			type = off_buf;
		}
				...
				struct ayla_tlv *name_tlv, size_t off, int del)
{
	...
	while (depth < plen) {
		...
		new = (struct conf_tree *)malloc(sizeof(*new));
		ASSERT(new);
		if (!new) {
...
extern uint32_t PIN_DYNAMIC_SW_LED;
//定义高低电平驱动
extern int flag_state_led;
extern int flag_sw_led;
extern int flag_relay;
extern int flag_button;
extern int flag_net_mode1;
extern int flag_net_mode2;
...
void io_init(int io_num,int mode){
	...
	switch(mode){
        case 1:
           PIN_DYNAMIC_STATE_LED=pin;
          
           GPIO_InitStruct.GPIO_Pin = PIN_DYNAMIC_STATE_LED;
           GPIO_InitStruct.GPIO_Mode = GPIO_Mode_OUT; //输出模式
           GPIO_InitStruct.GPIO_PuPd = GPIO_PuPd_NOPULL;
           GPIO_Init(&GPIO_InitStruct);

           GPIO_Direction(PIN_DYNAMIC_STATE_LED, GPIO_Mode_OUT);
           GPIO_PuPd = PullNone;
           PAD_PullCtrl(PIN_DYNAMIC_STATE_LED, GPIO_PuPd);
           break;//state_led
        case 2:
           PIN_DYNAMIC_SW_LED=pin;
	     
           GPIO_InitStruct.GPIO_Pin = PIN_DYNAMIC_SW_LED;
           GPIO_InitStruct.GPIO_Mode = GPIO_Mode_OUT; //输出模式
           GPIO_InitStruct.GPIO_PuPd = GPIO_PuPd_NOPULL;
           GPIO_Init(&GPIO_InitStruct);

           GPIO_Direction(PIN_DYNAMIC_SW_LED, GPIO_Mode_OUT);
           GPIO_PuPd = PullNone;
           PAD_PullCtrl(PIN_DYNAMIC_SW_LED, GPIO_PuPd);
           break;//sw_led
        case 3:
           PIN_DYNAMIC_KEY=pin;
         
           GPIO_InitStruct.GPIO_Pin  =  PIN_DYNAMIC_KEY;
           GPIO_InitStruct.GPIO_Mode =  GPIO_Mode_IN;//输入模式
           GPIO_InitStruct.GPIO_PuPd =  GPIO_PuPd_NOPULL;
           GPIO_Init(&GPIO_InitStruct);

           GPIO_Direction(PIN_DYNAMIC_KEY, GPIO_Mode_IN);
           GPIO_PuPd = GPIO_PuPd_UP;
           PAD_PullCtrl(PIN_DYNAMIC_KEY, GPIO_PuPd);
           break;
        case 4:
           PIN_DYNAMIC_OPT=pin;
         
           GPIO_InitStruct.GPIO_Pin = PIN_DYNAMIC_OPT;
           GPIO_InitStruct.GPIO_Mode = GPIO_Mode_OUT; //输出模式
           GPIO_InitStruct.GPIO_PuPd = GPIO_PuPd_NOPULL;
           GPIO_Init(&GPIO_InitStruct);

           GPIO_Direction(PIN_DYNAMIC_OPT, GPIO_Mode_OUT);
           GPIO_PuPd = PullNone;
           PAD_PullCtrl(PIN_DYNAMIC_OPT, GPIO_PuPd);
           break;
        default:break;
      }
}
...
void conf_cli(int argc, char **argv)
{
    ...
    if (argc == 2&& !strcmp(argv[1], "erase")){
     ...
     if (argc == 5&& !strcmp(argv[1], "io")&&!strcmp(argv[2], "state_led")){
        ...
        strcat(data,sign);
        strcat(data,argv[4]);
        printf("state_led-data:%s\n",data);
        u8 return_value=flash_stream_write(&flash,  ADDRESS, 50,data);
     ...
     if (argc == 5&& !strcmp(argv[1], "io")&&!strcmp(argv[2], "relay")){
        ...
        strcat(data,argv[4]);
        printf("relay-data:%s\n",data);
        u8 return_value=flash_stream_write(&flash,  ADDRESS+150, 50,data);
        char *argv2[] = { "conf", "save" };
        conf_cli(2, argv2);
        if(return_value==1){
          printf("\npara set ok\r\n");
     ...
     if (argc == 4&& !strcmp(argv[1], "cfg")){
      ...
      if (argc == 2&& !strcmp(argv[1], "getdata1")){
            ...
            printf("getdata1:argv1:%s-argv2:%s-argv3:%s\n",argv1,argv2,argv3);
            int io_num=atoi_io(argv2+4);
            printf("io_num:%d\n",io_num);
            io_init(io_num,1);//IO口初始化
            if(argv3[0]=='h'){ //高电平驱动
               flag_state_led=1;
      ...
      if (argc == 2&& !strcmp(argv[1], "getdata2")){
	        ...
	        temp = strtok(NULL,"***");
	        snprintf(argv3,strlen(temp)+1,"%s",temp);
	        printf("getdata2:argv1:%s-argv2:%s-argv3:%s\n",argv1,argv2,argv3);
            int io_num=atoi_io(argv2+4);
            printf("io_num:%d",io_num);
            io_init(io_num,2);//IO口初始化
            if(argv3[0]=='h'){ //高电平驱动
               flag_sw_led=1;
            }else if(argv3[0]=='l'){ //低电平驱动
               flag_sw_led=0;
     ...
     if (argc == 2&& !strcmp(argv[1], "getdata3")){
	        ...
	        temp = strtok(NULL,"***");
	        snprintf(argv3,strlen(temp)+1,"%s",temp);
	        printf("getdata3:argv1:%s-argv2:%s-argv3:%s\n",argv1,argv2,argv3);
            int io_num=atoi_io(argv2+4);
            printf("io_num:%d",io_num);
            io_init(io_num,3);//IO口初始化
            if(argv3[0]=='h'){ //高电平驱动
               flag_button=1;
            }else if(argv3[0]=='l'){ //低电平驱动
               flag_button=0;
      ...
      if (argc == 2&& !strcmp(argv[1], "getdata4")){
	        ...
	        temp = strtok(NULL,"***");
	        snprintf(argv3,strlen(temp)+1,"%s",temp);
	        printf("getdata4:argv1:%s-argv2:%s-argv3:%s\n",argv1,argv2,argv3);
            int io_num=atoi_io(argv2+4);
            printf("io_num:%d",io_num);
        ...
        if (argc == 2&& !strcmp(argv[1], "getdata5")){
	        ...
	        temp = strtok(NULL,"***");
	        snprintf(argv2,strlen(temp)+1,"%s",temp);
	        printf("getdata5:argv1:%s-argv2:%s\n",argv1,argv2);
            ...
            printf("getdata5:io_num1:%d--io_num2:%d\n",io_num1,io_num2);
            //取出MODE1和MODE2之后的动作
            flag_net_mode1=io_num1;
            flag_net_mode2=io_num2; 
        }
        ...
        if (argc == 2&& !strcmp(argv[1], "getdata6")){
	        ...
	        temp = strtok(NULL,"***");
	        snprintf(argv2,strlen(temp)+1,"%s",temp);
	        printf("getdata6:argv1:%s-argv2:%s\n",argv1,argv2);
            char temp_io_num1[2]=NULL; //数字
	        char temp_io_num2[2]=NULL; //数字
	        memcpy(temp_io_num1,argv1,2);
	        memcpy(temp_io_num2,argv2,2);
            int io_num1=atoi_io(temp_io_num1);
            int io_num2=atoi_io(temp_io_num2);
            printf("getdata6:io_num1:%d--io_num2:%d\n",io_num1,io_num2);
            //取出MODE1和MODE2之后的动作
            flag_cfg_net=io_num1;
            flag_cfg_relay=io_num2;
        }  
}
...
void conf_table_entry_add(const struct conf_entry *entry)
{
	...
	new = (struct conf_entry **)malloc((old_count + 2) * sizeof(*old));
	ASSERT(new);
	if (!new) {
		return;
	}
